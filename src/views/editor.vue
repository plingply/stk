  ...
  <div class="infobox">
    ...
    <div class="ctlist">
      ...
      <div class="row">
        ...
        <input type="number" v-model="info.total_count">
      </div>
    </div>
    ...
    <div class="bottombox">
      ...
      <span class="okbg" v-if="info.status == '1'" @click="editorStatus(2)">下线</span>
      <span class="okbg" v-if="info.status != '1'" @click="save(1)">发布</span>
    </div>

    <!-- 地图弹框	 -->
    <yd-popup v-model="mapshow" position="center" width="90%">
      <maps v-if="info.id" :height="400" :longitude="longitude" :latitude="latitude" :canclick="canclickBaidu" @getmappoint="getMapData"></maps>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(data => {
          ...
          if (data.status == "ok") {
            ...
            if (this.info.total_count > 0) {
              this.baomNumber = true;
            } else {
              this.baomNumber = false;
            }
          }
        });
    },
    ...
    save(type, isyl) {
      ...
      this.$http
        .post("/api/trial/trial_class/update/" + this.info.id, {
          data: this.info
        })
        .then(data => {
          this.$dialog.loading.close();
